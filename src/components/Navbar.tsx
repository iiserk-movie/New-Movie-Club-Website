...
export default function Navbar({
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full border-b border-zinc-800/80 bg-[#12111a]/98 backdrop-blur-lg shadow-xl shadow-black/40">
        <div className="mx-auto flex max-w-7xl min-h-[84px] py-3 items-center justify-between px-4 sm:px-6 lg:px-8">
          {/* Logo Brand */}
          <div className="flex items-center space-x-3.5 cursor-pointer group" onClick={() => setActiveTab('schedule')}>
            <div className="relative flex h-12 w-12 items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-300">
              <MovieClubLogo className="h-12 w-12" />
              <div className="absolute -top-0.5 -right-0.5 flex h-3 w-3 rounded-full bg-amber-500 animate-pulse ring-2 ring-zinc-950"></div>
            </div>
            <div>
              <h1 className="font-serif text-xl sm:text-2xl font-black tracking-wide text-transparent bg-clip-text bg-gradient-to-r from-zinc-100 via-amber-200 to-amber-400 drop-shadow-[0_2px_10px_rgba(245,158,11,0.25)] uppercase">
                Movie Club
              </h1>
              <p className="font-mono text-xs tracking-widest text-amber-400/90 font-bold uppercase">
                IISER Kolkata
              </p>
            </div>
          </div>
          ...
          {/* Navigation Items */}
          <nav className="hidden lg:flex items-center bg-[#181628]/80 border border-zinc-800/80 p-1.5 rounded-2xl shadow-lg shadow-black/50 backdrop-blur-md gap-1">
            {[
              ...
              return (
                <button
                  ...
                  className={`relative px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold tracking-wider uppercase transition-all duration-300 flex items-center gap-2 cursor-pointer ${
                    isActive
                      ? 'text-amber-300 bg-zinc-900/95 shadow-lg shadow-amber-500/10 border border-amber-500/30 font-bold scale-[1.02]'
          ...
          {/* Medium screen simplified navigation pills */}
          <nav className="hidden md:flex lg:hidden items-center bg-[#181628]/80 border border-zinc-800/80 p-1 rounded-xl shadow-lg shadow-black/50 gap-0.5">
            {[
              { id: 'schedule', label: 'Screenings', icon: Calendar },
              ...
              return (
                <button
                  ...
                  id={`tab-${tab.id}`}
                  onClick={() => setActiveTab(tab.id)}
                  className={`relative px-3 py-2 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all duration-300 flex items-center gap-1.5 cursor-pointer ${
                    isActive
                      ? 'text-amber-300 bg-zinc-900 shadow-md border border-amber-500/30 font-bold'
                      : 'text-zinc-300 hover:text-white hover:bg-zinc-800/60 border border-transparent'
                  }`}
                >
