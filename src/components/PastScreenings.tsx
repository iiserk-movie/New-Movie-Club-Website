...
import { 
  ...
  ChevronDown, ChevronUp, ChevronLeft, ChevronRight, User, Clock, Send, MessageCircleCode, CheckCircle2,
...
export default function PastScreenings({ 
  ...
  const filteredPastMovies = pastMovies.filter((movie) => {
    if (!searchQueryLower) return true;
    const titleMatch = movie.title.toLowerCase().includes(searchQueryLower);
    const directorMatch = movie.director.toLowerCase().includes(searchQueryLower);
    const yearMatch = String(movie.year).includes(searchQueryLower);
    const dateMatch = (movie.screenedDate || '').toLowerCase().includes(searchQueryLower);
    const genreMatch = movie.genre?.some((g) => g.toLowerCase().includes(searchQueryLower));
    return titleMatch || directorMatch || yearMatch || dateMatch || genreMatch;
  });

  // Pagination calculations
  const totalPages = Math.max(1, Math.ceil(filteredPastMovies.length / ITEMS_PER_PAGE));
  const validCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (validCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentPaginatedMovies = filteredPastMovies.slice(startIndex, startIndex + ITEMS_PER_PAGE);

  const handlePageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= totalPages) {
      setCurrentPage(newPage);
      const element = document.getElementById('past-screenings-grid');
      if (element) {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="relative rounded-3xl overflow-hidden h-48 md:h-60 mb-10 border border-amber-500/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-end">
        ...
        <div className="relative z-10 p-6 md:p-8 max-w-2xl space-y-1.5">
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <span className="bg-amber-500/10 text-amber-400 border border-amber-500/20 font-mono text-[10px] font-bold px-2.5 py-0.5 rounded-full shadow-sm flex items-center gap-1">
              <span>🎬</span>
              <span>{pastMovies.length} Movies Screened</span>
            </span>
          </div>
          <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl font-bold text-zinc-100 tracking-tight flex flex-wrap items-center gap-3">
      ...
      {/* Search Bar & Pagination Controls Header */}
      <div id="past-screenings-grid" className="pt-2 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-zinc-900/60 border border-zinc-800/80 p-4 rounded-2xl backdrop-blur-md shadow-lg">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search past screenings by title, director, year, genre..."
              className="w-full bg-zinc-950/80 border border-zinc-800 text-zinc-100 placeholder-zinc-500 text-xs rounded-xl pl-10 pr-9 py-2.5 focus:outline-none focus:border-amber-500/60 transition-colors"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          {/* Count & Top Pagination Indicator */}
          <div className="flex flex-wrap items-center justify-between md:justify-end gap-3 text-xs font-mono text-zinc-400">
            <span className="text-zinc-400">
              {filteredPastMovies.length === 0 ? (
                <span className="text-amber-500/90 font-medium">No screenings found</span>
              ) : (
                <>
                  Showing <strong className="text-zinc-200">{startIndex + 1}</strong>-
                  <strong className="text-zinc-200">
                    {Math.min(startIndex + ITEMS_PER_PAGE, filteredPastMovies.length)}
                  </strong> of <strong className="text-amber-400">{filteredPastMovies.length}</strong>
                  {searchQuery && <span className="ml-1 text-zinc-500">(filtered)</span>}
                </>
              )}
            </span>

            {/* Top Pagination Buttons (1, 2, 3...) */}
            {totalPages > 1 && (
              <div className="flex flex-wrap items-center gap-1 bg-zinc-950/80 p-1 rounded-xl border border-zinc-800">
                <button
                  type="button"
                  onClick={() => handlePageChange(validCurrentPage - 1)}
                  disabled={validCurrentPage === 1}
                  className="p-1.5 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-850 disabled:opacity-30 disabled:hover:bg-transparent transition-colors cursor-pointer disabled:cursor-not-allowed"
                  title="Previous Page"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {currentPaginatedMovies.map((movie) => {
      ...
      {filteredPastMovies.length === 0 && (
        <div className="text-center py-16 bg-zinc-900/30 border border-zinc-850 rounded-3xl space-y-3">
          <Search className="h-10 w-10 text-zinc-600 mx-auto" />
          <h3 className="font-serif text-lg font-bold text-zinc-300">No screenings match "{searchQuery}"</h3>
          <p className="text-xs text-zinc-500 max-w-sm mx-auto">
            Try searching with a different movie title, director's name, or release year.
          </p>
          <button
            type="button"
            onClick={() => {
            ...
            className="mt-2 inline-flex items-center space-x-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/20 px-4 py-2 rounded-xl text-xs font-mono transition-colors cursor-pointer"
          >
      ...
      {totalPages > 1 && filteredPastMovies.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-zinc-900">
          <span className="text-xs font-mono text-zinc-500">
            Page <strong className="text-amber-400">{validCurrentPage}</strong> of <strong className="text-zinc-300">{totalPages}</strong> ({filteredPastMovies.length} total past screenings)
          </span>
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2">
            <button
              ...
              onClick={() => handlePageChange(validCurrentPage - 1)}
              disabled={validCurrentPage === 1}
              className="flex items-center space-x-1 bg-zinc-900 hover:bg-zinc-850 text-zinc-300 border border-zinc-800 disabled:opacity-40 px-3 py-2 rounded-xl text-xs font-mono font-medium transition-all cursor-pointer disabled:cursor-not-allowed"
            >
              <ChevronLeft className="h-4 w-4" />
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                key={`bottom-page-btn-${page}`}
                type="button"
                onClick={() => handlePageChange(page)}
                className={`h-9 min-w-[34px] px-2.5 rounded-xl font-mono text-xs font-bold transition-all cursor-pointer ${
                  validCurrentPage === page
