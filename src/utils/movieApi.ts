...
export async function getMovieDetails(movieQuery: string): Promise<MovieDetails> {
  ...
  try {
    ...
    console.log('[Movie API] Backend /api/movie-details inaccessible. Falling back to Client-Side AI Engine.', err);
  }

  // 2. Client-side backend fallback (GitHub Pages)
...
export async function searchMovies(query: string): Promise<SearchMovieResult[]> {
  ...
  const apiKey = getLocalGeminiKey();
  if (!apiKey) {
    return []; // Fail silently on autocomplete if key is missing
  }
